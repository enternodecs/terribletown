...
hook.Add( "EntityTakeDamage", "AdditionalDamage", function(target, dmginfo) 
    local victim = target
    local attacker = dmginfo:GetAttacker()
    local weapon = dmginfo:GetInflictor()
    local damage = dmginfo:GetDamage()
    if weapon:IsPlayer() then return end
    if !attacker:IsPlayer() then return end --In case some jackass spawns an NPC and gives it the gun. This is a TTT weapon...
    local new = damage - (attacker:GetPos():Distance( victim:GetPos() ) * (.05/damage))
    dmginfo:SetDamage( new )
    --print(attacker, " attacked ", victim, " with ", weapon, " for ", new)
end)

function SWEP:GetPrimaryCone()
   local cone = self.Primary.Cone or 0.2
   local movedet = self.MovingInaccuracyMult / 200
   cone = cone + (movedet*self.Owner:GetVelocity():Length()*cone)
   if self.Owner:Crouching() then cone = cone * self.CrouchingBonus end
   if self:GetIronsights() then cone = cone * self.ADSBonus end
   if !self.Owner:IsOnGround() then cone = cone * self.JumpingInaccuracyMult end
